...
function query(filterBy) {
    ...
    if (filterBy) {
        var { type, minBattery, maxBattery } = filterBy
        maxBattery = maxBattery || Infinity
        minBattery = minBattery || 0
        robotsToReturn = gRobots.filter(robot => robot.type.toLowerCase().includes(type.toLowerCase())
